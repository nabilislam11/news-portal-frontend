          ...
          <div className="relative">
            <div className="w-10 h-10 bg-gradient-to-br from-red-500 to-red-600 rounded-lg flex items-center justify-center shadow-lg shadow-red-500/50 transition-all duration-300">
              ...
              <div className="relative w-6 h-6">
                {/* Animated lines */}
                <div className={`absolute left-0 top-0 w-0.5 h-6 bg-white rounded-full transition-all duration-300 ${isHovered ? 'h-7 -top-0.5' : ''}`}></div>
                <div className={`absolute left-0 top-0 w-6 h-0.5 bg-white rounded-full origin-left transform rotate-[135deg] transition-all duration-300 ${isHovered ? 'w-7' : ''}`}></div>
                <div className={`absolute right-0 top-0 w-0.5 h-6 bg-white rounded-full transition-all duration-300 ${isHovered ? 'h-7 -top-0.5' : ''}`}></div>
