import {  useUpdateCategory } from "@/api/hooks/category";
...
export function EditCategory({data}:{data:Category}) {
  ...
  const catMutation = useUpdateCategory();
  ...
  const handleSubmit = (values: Category) => {
    console.log(values);
    catMutation.mutate({id:values._id as string,...values}, {
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: ["categories", values._id] });
  ...
  return (
    <Dialog open={open} onOpenChange={isOpen}>
      <DialogTrigger asChild>
        <Button size={"sm"} className="bg-blue-500 text-white"><Pencil/></Button>
      </DialogTrigger>
