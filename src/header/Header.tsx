    ...
    <header className="w-full bg-white shadow-md fixed top-9 left-0 z-50 shadow-gray-200">
      ...
      <Container>
        <div className="w-full">
          <div className="mx-auto py-5 flex justify-between items-center">
            ...
            <div className="hidden md:flex gap-6 items-center text-gray-700 font-medium">
              ...
              <Link to="/categories-all">
                <RiGalleryView2 className="ml-2 text-gray-700 " size={30} />
              </Link>
            </div>
            {/* Search Section */}
          ...
          {/* Mobile Menu Content */}
          {isOpen && (
            <div className="md:hidden bg-white border-t border-gray-200 py-3 grid grid-cols-2 gap-2 px-2">
              {Array.isArray(navItems) &&
                navItems.map((nav) => (
                  <Link
                    key={nav._id}
              ...
              {/* এই h1 ট্যাগটি এখন ২ কলাম জুড়ে থাকবে এবং সেন্টারে দেখাবে */}
              <Link
                to="/categories-all"
                onClick={() => setIsOpen(false)}
                className="col-span-2 font-semibold hover:text-red-600 p-2 text-center border rounded-md bg-gray-50 active:scale-95 transition-all"
              >
                All Category
              </Link>
            </div>
